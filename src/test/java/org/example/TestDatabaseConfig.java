...
public class TestDatabaseConfig {
	...
	public static void withDslContext(Consumer<DSLContext> testLogic) throws Exception {
		...
		try (final Connection connection = DriverManager.getConnection(config.url(), config.username(),
				config.password())) {
	...
	public static DatabaseConnectionSettings load() {
		try (final InputStream input = TestDatabaseConfig.class.getClassLoader()
